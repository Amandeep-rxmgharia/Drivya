...
const card =
  "rounded-2xl glass shadow-elegant";
const subtleHover =
...
const navWorkspace = [
  { icon: Users, label: "Team Workspace", to: "team", badge: "Pro" },
  { icon: ShieldCheck, label: "Secure Vault", to: "vault" },
];
        ...
        {!collapsed && (
          <div className="m-3 rounded-2xl glass p-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-foreground/90">Storage</span>
              <span className={chip}>
...
function Topbar({ onToggleSidebar, onMobileMenu, sidebarCollapsed }) {
  ...
  return (
    <header className="sticky top-0 z-20 bg-background/60 backdrop-blur-xl border-b border-border/70">
      <div className="h-16 flex items-center gap-3 px-4 md:px-6">
        <button onClick={onMobileMenu} className={`${iconBtn} lg:hidden`}>
          <Menu className="h-4 w-4" />
        </button>
        <button onClick={onToggleSidebar} className={`${iconBtn} hidden lg:inline-flex`}>
        ...
        <div className="hidden xl:flex items-center gap-3 rounded-xl border border-border bg-secondary/30 px-3 h-10">
          ...
          <span className="text-xs font-medium text-muted-foreground">156 / 250 GB</span>
        </div>
          ...
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-border bg-popover shadow-elegant p-2">
              <div className="px-3 py-2.5">
                <div className="text-sm font-semibold text-foreground">Amelia Moreau</div>
                <div className="text-xs text-muted-foreground">amelia@drivya.com</div>
              </div>
...
function HeroSection() {
  return (
    <section className={`${card} ${subtleHover} relative overflow-hidden p-6 md:p-10`}>
      ...
      <div className="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10">
        <div className="max-w-xl">
          ...
          <h1 className="mt-5 font-display text-4xl md:text-5xl font-semibold tracking-tight leading-[1.1] text-foreground">
            Good morning,{" "}
            <span className="text-gradient">Amelia</span>.
          </h1>
          <p className="mt-3 text-muted-foreground leading-relaxed max-w-lg">
            You uploaded <span className="font-semibold text-foreground">42 files</span> this week —
            12% more than last week. Your vault is fully encrypted and synced across 3 devices.
          </p>
      ...
      <div className="relative h-36 w-36">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="font-display text-2xl font-semibold tracking-tight text-foreground">
            156<span className="text-muted-foreground text-sm font-medium"> GB</span>
          </div>
          <div className="text-[11px] font-medium text-muted-foreground">of 250 GB</div>
        </div>
      </div>
      <div className="space-y-2 min-w-[140px]">
        {[
          ...
          <div key={t.l} className="flex items-center gap-2 text-xs">
            ...
            <span className="font-semibold text-foreground tabular-nums">{t.v} GB</span>
          </div>
        ))}
      </div>
    </div>
...
function AnalyticsCard() {
  ...
  return (
    <div className={`${card} ${subtleHover} p-6`}>
      <div className="flex items-center justify-between">
        <div>
        ...
        <button className={iconBtn}><MoreHorizontal className="h-4 w-4" /></button>
      </div>
      <div className="mt-6 flex items-end gap-3 h-32">
        {days.map((v, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-2">
            ...
            />
            <span className="text-[10.5px] font-medium text-muted-foreground/80">{labels[i]}</span>
          </div>
        ))}
      </div>
    </div>
...
const files = [
  { name: "Brand Guidelines v3.pdf", icon: FileText, type: "PDF", owner: "Amelia M.", modified: "2h ago", size: "4.2 MB" },
  { name: "Q4-keynote-final.mp4", icon: Film, type: "Video", owner: "Lukas R.", modified: "Yesterday", size: "128 MB" },
  { name: "Hero-shot-005.png", icon: ImageIcon, type: "Image", owner: "Amelia M.", modified: "Mon", size: "8.1 MB" },
  { name: "investor-deck.key", icon: FileText, type: "Doc", owner: "Maya P.", modified: "Mar 12", size: "12.4 MB" },
  { name: "client-archives.zip", icon: FileArchive, type: "Archive", owner: "Amelia M.", modified: "Mar 11", size: "640 MB" },
];
  ...
  return (
    <div className={`${card} overflow-hidden`}>
      <div className="flex items-center justify-between p-6 pb-4">
        <div>
          <h3 className="font-display text-base font-semibold text-foreground">Recent files</h3>
          <p className="text-xs text-muted-foreground mt-0.5">Across all your folders</p>
        </div>
      ...
      <div className="px-3 pb-3">
        ...
        {files.map((f, i) => (
          <motion.div
            ...
            transition={{ duration: 0.5, delay: i * 0.04, ease: easeSmooth }}
          ...
          >
            ...
            <div className="hidden md:block text-sm text-muted-foreground">{f.modified}</div>
            <div className="hidden md:block text-sm text-muted-foreground tabular-nums">{f.size}</div>
            <div className="hidden md:flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <button className={iconBtn} title="Share"><Share2 className="h-4 w-4" /></button>
              <button className={iconBtn} title="Download"><Download className="h-4 w-4" /></button>
              <button className={iconBtn} title="Star"><Star className="h-4 w-4" /></button>
              <button className={`${iconBtn} hover:text-destructive`} title="Delete"><Trash2 className="h-4 w-4" /></button>
            </div>
          </motion.div>
        ))}
      </div>
...
const uploads = [
  { name: "campaign-cut-final.mp4", size: "284 MB", progress: 72, speed: "8.4 MB/s", state: "uploading" },
  { name: "asset-pack-2025.zip", size: "1.2 GB", progress: 41, speed: "12.1 MB/s", state: "uploading" },
  { name: "logo-master.ai", size: "44 MB", progress: 100, speed: "—", state: "done" },
  { name: "annual-report.pdf", size: "62 MB", progress: 0, speed: "—", state: "failed" },
];
...
function UploadQueue() {
  return (
    <div className={`${card} p-6`}>
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-display text-base font-semibold text-foreground">Upload queue</h3>
          <p className="text-xs text-muted-foreground mt-0.5">2 active · avg 20.5 MB/s</p>
        </div>
        <button className={iconBtn}><X className="h-4 w-4" /></button>
      </div>

      <div className="mt-4 rounded-2xl border border-dashed border-border bg-secondary/20 p-6 text-center hover:bg-secondary/40 hover:border-primary/40 transition-colors cursor-pointer">
        <UploadCloud className="h-6 w-6 text-primary mx-auto drop-shadow-primary-glow" />
        <div className="mt-2 text-sm font-medium text-foreground">
          Drop files here or <span className="text-primary">browse</span>
        </div>
        <div className="text-[11px] text-muted-foreground">Max 5 GB per file · End-to-end encrypted</div>
      </div>

      <ul className="mt-4 space-y-3">
        {uploads.map((u) => (
          <li key={u.name} className="rounded-xl border border-border/60 bg-secondary/20 p-3">
            <div className="flex items-center gap-3">
              ...
              {u.state === "failed" && (
                <button className="text-xs font-semibold text-foreground hover:text-primary transition-colors">Retry</button>
              )}
            </div>
          </li>
        ))}
  ...
  return (
    <div className={`${card} p-6`}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display text-base font-semibold text-foreground">Shared with team</h3>
        ...
        {shared.map((s) => (
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="mt-5 text-sm font-semibold text-foreground">{s.name}</div>
...
const activities = [
  { icon: UploadCloud, text: "You uploaded 12 files to Brand kit 2025", when: "10 min ago" },
  { icon: Share2, text: "Shared Q3 financials with finance@drivya.com", when: "1 hr ago" },
  { icon: LogIn, text: "New login from MacBook Pro · Lisbon, PT", when: "3 hr ago" },
  { icon: ShieldCheck, text: "End-to-end encryption enabled for Vault", when: "Yesterday", accent: true },
  ...
  return (
    <div className={`${card} p-6`}>
      <div className="flex items-center justify-between mb-5">
        <h3 className="font-display text-base font-semibold text-foreground">Activity</h3>
        <span className={chip}>
          <span className="h-1.5 w-1.5 rounded-full bg-primary shadow-glow" /> Live
        </span>
      </div>
          ...
          <li key={i} className="relative flex gap-3">
            <div
              className={`relative z-10 h-8 w-8 rounded-full ring-4 ring-background flex items-center justify-center border border-border/60 ${
                a.accent ? "bg-gradient-primary text-primary-foreground shadow-glow" : "bg-secondary/70 text-foreground/80"
              }`}
            >
    ...
    <div className={`${card} relative overflow-hidden p-6`}>
      ...
      <div className="relative mt-5 grid grid-cols-2 gap-2.5">
        {[
          ...
          <div key={b.l} className="flex items-center gap-2 rounded-xl border border-border/60 bg-secondary/20 px-3 py-2.5">
            <b.icon className="h-4 w-4 text-primary" />
            <span className="text-xs font-medium text-foreground/90">{b.l}</span>
          </div>
        ))}
      </div>
      ...
      <div className="relative mt-5">
        ...
        <ul className="space-y-2">
          {[
            ...
            <li key={s.d} className="flex items-center justify-between rounded-xl border border-border/60 bg-secondary/20 px-3 py-2.5">
              <div>
              ...
              {s.current ? (
                ...
                <button className="text-[11px] font-semibold text-muted-foreground hover:text-destructive transition-colors">Revoke</button>
              )}
            </li>
          ))}
        </ul>
          ...
          <main className="p-4 md:p-6 lg:p-8 space-y-6">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground/80 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span>Press <Kbd>⌘</Kbd> <Kbd>K</Kbd> to search · <Kbd>U</Kbd> to upload</span>
              </div>
