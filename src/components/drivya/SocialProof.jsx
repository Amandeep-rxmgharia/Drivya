    ...
    <Section className="py-16 md:py-20">
      ...
      <div className="mt-8 flex flex-wrap justify-center items-center gap-x-10 gap-y-6 opacity-70">
        {logos.map((l) => (
          <span
            key={l}
            className="font-display text-xl md:text-2xl tracking-tight text-muted-foreground/80 hover:text-foreground transition-colors"
          >
            {l}
          </span>
        ))}
      ...
      <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ type: "tween", duration: 0.68, ease: easeSmooth, delay: i * 0.08 }}
