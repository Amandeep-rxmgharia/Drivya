...
export function ShareModal({ file, onClose, onShareUpdated }) {
  ...
  // One-time generated password modal removed (password is shown only via notification inbox)
  ...
  const loadShare = useCallback(async () => {
    ...
    try {
      const { share, password } = await createShare({ resourceId: file.id });
      setShareId(share.id);
      setShareLinkUrl(share.fullLinkUrl || `http://${share.linkUrl}`);
  ...
  return (
    <>
      {/* Main modal portal content */}
      {/* (existing share settings UI below) */}
      ...
      {createPortal(
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-background/40 backdrop-blur-[4px]"
            onClick={() => {
              onClose();
            }}
          />
          ...
          <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center sm:p-4 pointer-events-none">
            <motion.div
                        ...
                        {activeTab === "collaborators" ? (
                          ...
                          >
                            ...
                            <div className="rounded-2xl border border-border/60 bg-secondary/10 p-3.5 sm:p-4 space-y-4 w-full">
                              ...
                              {/* Permissions Switch Card */}
                              <div
                                className={cn(
                                  "flex flex-col gap-2 border border-border/40 bg-background/25 rounded-2xl p-3 text-xs transition-all",
                                  !linkActive && "opacity-50 pointer-events-none",
                                )}
                              >
                                <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                                ...
                                {visibility === "Public" ? (
                                  <div className="flex items-start gap-2.5 border border-dashed border-emerald-500/25 bg-emerald-500/5 rounded-2xl px-3.5 sm:px-4 py-3 text-[10px] text-muted-foreground/80 font-medium leading-relaxed">
                                    ...
                                    <span>
                                      ...
                                      Switch the Visibility mode to{" "}
                                      <strong>Restricted</strong> to restrict public
                                      access with password security.
                                    </span>
                                  </div>
                                ) : (
                                          ...
                                          <div className="flex flex-col sm:flex-row gap-2">
                                            <div className="relative flex-1">
                                              <input
                                                ...
                                                className="h-11 sm:h-10 w-full rounded-xl border border-border/80 bg-background/50 pl-3.5 pr-3 text-xs text-foreground focus:outline-none focus:border-primary/50 transition-all font-mono"
                                              />
                                            </div>
