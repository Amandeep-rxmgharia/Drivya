...
export function DropboxModal({
  ...
  // ── Auth state ───────────────────────────────────────────────
  const { enqueueDropboxImports, isCloudImporting } = useUpload();
    ...
    window.dispatchEvent(
      new CustomEvent("add-drivya-notification", {
        detail: {
          ...
          description: `Started importing ${filesToImport.length} file${filesToImport.length > 1 ? "s" : ""} from Dropbox in background.`,
          type: "dropbox",
